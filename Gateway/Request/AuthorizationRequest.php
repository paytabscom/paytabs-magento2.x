...
class AuthorizationRequest implements BuilderInterface
{
    ...
    public function build(array $buildSubject)
    {
        ...
        if (!$preorder) {
            ...
            ClickpayHelper::log("Validate Auth!, Order [{$order_id}], Amount {$amount} {$currency}, Transaction {$transaction_registered}", 1);

            //

            if (!$transaction_registered) {
                ClickpayHelper::log("Validate Auth!, tran_ref should be provided", 3);
